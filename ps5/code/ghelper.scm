...
(pp (get-operator-record foo))
(3 (b . one-arg-b) . foo-default)

(defhandler foo 'two-arg-a-b 'a 'b)

(pp (get-operator-record foo))
(3 (a (b . two-arg-a-b)) (b . one-arg-b) . foo-default)

(defhandler foo 'one-arg-a 'a)

(pp (get-operator-record foo))
(3 (a (b . two-arg-a-b) . one-arg-a) (b . one-arg-b) . foo-default)

(defhandler foo 'one-arg-a-prime 'a)
;Warning: Replacing a default handler: one-arg-a one-arg-a-prime

(pp (get-operator-record foo))
(3 (a (b . two-arg-a-b) . one-arg-a-prime)
   (b . one-arg-b)
   . foo-default)
...
(pp (get-operator-record foo))
(3 (a (b . two-arg-a-b-prime) . one-arg-a-prime)
   (b . one-arg-b)
   .
   foo-default)
